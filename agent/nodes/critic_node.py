...
def _deterministic_issues(state: FinancialAgentState) -> list[str]:
    ...
    if not cards:
        issues.append("证据不足：没有检索到可引用的证据卡片。")
    ...
    if any(pattern in draft for pattern in INVESTMENT_ADVICE_PATTERNS):
        issues.append("合规风险：草稿包含确定性投资建议或收益承诺。")
...
def critic_node(state: FinancialAgentState) -> dict:
    ...
    try:
        ...
        result["needs_more_evidence"] = bool(
            ...
            or (merged_issues and state.get("reflection_round", 0) < 1)
        )
    except Exception as exc:
        result = {
            "passed": not deterministic,
            "issues": deterministic,
            "needs_more_evidence": bool(deterministic and state.get("reflection_round", 0) < 1),
            "llm_used": False,
            "llm_error": str(exc),
        }
